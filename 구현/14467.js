...
const positionArray = Array.from({ length: 10 }, () => null);
let result = 0;

input.forEach((arr) => {
	const [number, position] = arr.split(' ');

	if (positionArray[number - 1] === null) {
		positionArray[number - 1] = position;
	} else {
		if (positionArray[number - 1] !== position) {
			result += 1;
			positionArray[number - 1] = position;
		}
	}
