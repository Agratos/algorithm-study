 ...
 * 문제
 *  오늘도 호재만 있게 해주세요. 버핏-"
 * 2년차 개미 준현이는 오늘도 버핏신에게 기도를 올린다. 
 * 장기 투자를 지향하는 준현이는 한 번 산 주식은 절대 팔지 않는다. 
 * 2099년이 되어도 주식을 팔지 않을 것이다. 
 * 주식 매수 후 오로지 기도만 하기 때문에 이를 BNP 전략이라고 한다. BNP는 Buy and Pray의 약자이다. 
 ...
 * 성민이는 주식이 타이밍 싸움이라 생각한다. 전형적인 단기 투자자로 생각하면 오산이다. 성민이만의 전략이 있기 때문이다. 
 ...
 * 우리는 2021년 1월 1일부터 2021년 1월 14일까지의 주식 가격을 미리 알고 있다. 
...
const inputArray = input.split(' ').map((str) => Number(str));

let bMony = Number(n);
let bCount = 0;
let tMony = Number(n);
let tCount = 0;
let stockTendencyUp = 0;
let stockTendencyDown = 0;

inputArray.forEach((stock, index) => {
	// 준현
	if (stock <= bMony) {
		bCount += Math.floor(bMony / stock);
		bMony = bMony % stock;
	}
	...
	if (index > 0) {
		...
		if (stockTendencyDown === -3) {
			// 전량 매수
			tCount += Math.floor(tMony / stock);
			tMony = tMony % stock;
			stockTendencyDown = 0;
		}
	}
...
bMony += bCount * inputArray[13];
tMony += tCount * inputArray[13];
